...
public class ClassificadoDAO extends DAO<Classificado> {
	...
	private Classificado serializeByCursor(Cursor cursor) {
		...
		classificado.setAreaInteresse(areainteresseDAO.getByID(cursor.getInt(5)));
		UsuarioDAO usuarioDAO = new UsuarioDAO(this.context);
		classificado.setUsuario(usuarioDAO.getById((cursor.getInt(6))));
	...
	private ContentValues serializeContentValues(Classificado classificado) {
		...
		values.put("AreaInteresse_idAreaInteresse", classificado.getAreaInteresse().getIdAreaInteresse());
		values.put("Usuario_idUsuario", classificado.getUsuario().getIdUsuario());
